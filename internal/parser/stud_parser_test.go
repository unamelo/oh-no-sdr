...
func TestSTUDParser_InvalidData(t *testing.T) {
	...
	// Short lines should now be handled gracefully (padded)
	shortContent := "1234567890123 F01011990  9999Y"

	records, err := parser.Parse(shortContent)
	if err != nil {
		t.Errorf("Short line should be handled gracefully: %v", err)
	}

	if len(records) != 1 {
		t.Errorf("Expected 1 record from short line, got %d", len(records))
	}
}
